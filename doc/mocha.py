...
def mini_mocha_spec(noise='bgs0_legacy', sample='mini_mocha', method='ifsps'): 
    ...
    specs, meta = Data.Spectra(sim='lgal', noise=noise_spec, lib='bc03', sample=sample) 
...
def mini_mocha_photo(noise='bgs0_legacy', sample='mini_mocha', method='ifsps'): 
    ...
    _, meta = Data.Photometry(sim='lgal', noise=noise_photo, lib='bc03', sample=sample) 
...
def mini_mocha_specphoto(noise='bgs0_legacy', sample='mini_mocha', method='ifsps'): 
    ''' Compare properties inferred from forward modeled photometry to input properties
    '''
    if noise != 'none':
        noise_spec = noise.split('_')[0]
    ...
    photo, _ = Data.Photometry(sim='lgal', noise=noise_photo, lib='bc03', sample=sample)
...
def Fbestfit_spec(igal, noise='none', sample='mini_mocha', method='ifsps'): 
    ...
    f_bf = os.path.join(UT.lgal_dir(), sample, method,'lgal.spec.noise_%s.%s.%i.hdf5' % (noise_spec, model, igal))
    return f_bf


def Fbestfit_photo(igal, noise='bgs0_legacy', sample='mini_mocha', method='ifsps'): 
    ''' file name of best-fit of photometry of spectral_challenge galaxy #igal
    :param igal: 
        index of spectral_challenge galaxy 
    :param noise:
    ...
    if noise != 'none':
        ...
        noise_photo = 'none'
    model = 'vanilla'   
    f_bf = os.path.join(UT.lgal_dir(), sample, method, 'lgal.photo.noise_%s.%s.%i.hdf5' % (noise_photo, model, igal))
    return f_bf


def Fbestfit_specphoto(igal, noise='bgs0_legacy', sample='mini_mocha', method='ifsps'): 
    ''' file name of best-fit of photometry of spectral_challenge galaxy #igal
    :param igal: 
        index of spectral_challenge galaxy 
    :param noise:
    ...
    '''
    model = 'vanilla'   
    f_bf = os.path.join(UT.dat_dir(), sample , 'ifsps', 'lgal.specphoto.noise_%s.%s.%i.hdf5' % (noise, model, igal))
    return f_bf
...
if __name__=="__main__": 
    ...
    #mini_mocha_photo(noise='bgs0_legacy', method='ifsps')
    mini_mocha_spec(noise='bgs0_legacy', method='ifsps')
